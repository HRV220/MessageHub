...
| 2.1    | Упрощение объёма v1 для MVP. Поиск (US-09) и уведомления (US-10) полностью исключены из v1 и перенесены в раздел 20 «Перспективы развития» (ОВ-05, ОВ-09 закрыты). Отдельный раздел UI «Новые диалоги» убран: новые диалоги (`Pending`) обрабатываются через повторное открытие экрана «Выбор кандидатов» (US-08) без изменения доменной модели. Настройки (US-03) сокращены: убраны настраиваемое расположение файла БД (НФТ-11), автозапуск с Windows, правила хранения истории, настраиваемый уровень логирования (оставлено вкл/выкл). Каналы связи (US-02) и merge/split собеседников (US-05) оставлены без изменений. |
...
| Идентичность в канале | Учётная запись собеседника в конкретном канале (`ChannelContact`): `@ivan_petrov` в Telegram, `ivan@example.com` в Email.                                          |
...
- [ ] Токены и пароли каналов хранятся только в памяти процесса на время его работы и никогда не записываются на диск — ни в открытом, ни в зашифрованном виде.
...
- [ ] Для каждого аккаунта отображается состояние: Connected / Disconnected / Error, дата подключения, дата последней синхронизации.
...
- [ ] Секрет (токен/пароль) аккаунта хранится только в памяти процесса; после перезапуска приложения все ранее подключённые аккаунты отображаются как Disconnected и требуют повторной авторизации.
...
**Трактовка (v2.1).** Объём настроек сокращён до необходимого для MVP. Расположение файла БД, автозапуск с Windows, правила автоматического удаления истории и настраиваемый уровень логирования исключены из v1 (раздел 20) — файл БД лежит в стандартной папке данных приложения, история не удаляется автоматически.

**Критерии приёмки:**

- [ ] Общие настройки: язык, тема, интервал синхронизации.
...
- [ ] Техническое логирование можно включить или выключить (без настройки уровня в v1).
- [ ] Изменения применяются без перезапуска.
- [ ] Для каждой настройки есть разумное значение по умолчанию.

**Связанные требования:** ФТ-301, ФТ-303, ФТ-304, ФТ-307, ФТ-308.
...
**Трактовка.** Приложение **не импортирует автоматически** все диалоги канала. После подключения и в любой момент позже пользователь видит список кандидатов и отмечает нужных. Сообщения сохраняются только для добавленных диалогов. Диалоги, появившиеся позже при фоновой синхронизации, сохраняются как `Pending` (только метаданные) и ждут решения пользователя.

**Трактовка (v2.1).** Отдельного раздела UI «Новые диалоги» в v1 нет (вынесен в раздел 20). Диалоги в статусе `Pending` доступны через тот же экран «Выбор кандидатов»: при повторном открытии он показывает как новых кандидатов из адаптера, так и уже накопленные `Pending`-диалоги — в тех же вкладках по типу, с той же фильтрацией. Игнорирование и возврат работают там же, без отдельного списка.
...
- [ ] Экран выбора кандидатов можно открыть повторно из раздела «Каналы»; при повторном открытии в нём же видны диалоги, появившиеся после подключения (`Pending`), с действиями «Добавить» и «Игнорировать». Проигнорированные не предлагаются повторно, но их можно найти и вернуть (фильтр «Игнорированные»).
...
| 3   | Синхронизируются все диалоги включённых типов                          | US-08: пользователь сам выбирает, кого добавить                 | Выборочное добавление кандидатов. Флажки типов диалогов стали фильтрами экрана выбора, а не правилом автоимпорта. Новые диалоги (`Pending`) обрабатываются через повторное открытие того же экрана выбора (v2.1, без отдельного раздела UI).                                                                                  |
...
| 5   | Пароль необязателен, экран ввода пароля при старте                     | US-01: цель — конфиденциальность                                | Необязательность сохранена («возможность»). Добавлены: защита локального API сессией, проверка Origin, блокировка, медленная KDF. Шифрование файла БД вынесено в ОВ-01.                             |
...
| 8   | Поиск — обязательный раздел                                            | Нет user story                                                  | В v2.0 понижен до «Желательно» (US-09, раздел 4.2). В v2.1 исключён из v1 полностью, перенесён в раздел 20 «Перспективы развития».                                                                                                    |
| 9   | Уведомления — глобальный переключатель                                 | Нет user story                                                  | В v2.0 понижены до «Желательно» (US-10). В v2.1 исключены из v1 полностью, перенесены в раздел 20 «Перспективы развития».                                                                                                             |
...
### 4.2. Дополнительные user stories (вне v1)

Рассматривались как дополнительные user stories. Решением v2.1 обе исключены из объёма v1 и перенесены в раздел 20 «Перспективы развития» без привязки к конкретной версии.

- **US-09 (вне v1).** Как пользователь я хочу искать по собеседникам и тексту сообщений, чтобы быстро находить нужную информацию.
- **US-10 (вне v1).** Как пользователь я хочу получать уведомления о новых сообщениях, чтобы не пропускать важное.

### 4.3. Итог проверки

- Каждая user story объёма v1 покрыта функциональными требованиями и критериями приёмки (раздел 18).
- Требований, противоречащих user stories, в ТЗ v2.1 не осталось.
- Требования без user story либо системные, либо вынесены за пределы v1 (раздел 20).
...
| `ChannelContact`          | Сущность внутри агрегата `Person` | Id           | Учётная запись собеседника в канале. Сущность, потому что переносится при merge/split и на неё ссылаются диалоги и журнал. |
...
1. `ChannelContact` принадлежит ровно одному `Person`.
2. Пара `(ConnectedAccountId, ExternalId)` уникальна для `ChannelContact` и для `Conversation`.
3. Личный диалог в статусе `Added` связан с `ChannelContact` собеседника.
...
| НФТ-11 | Расположение файла БД фиксировано (стандартная папка данных приложения). Настраиваемое расположение — вне v1 (раздел 20). | Вне v1      |
...
| НФТ-16 | Поле `channel_type` в `messages` — осознанная денормализация для фильтра по каналу без JOIN. Не удалять при рефакторинге. | Обязательно |
...
Токены, пароли и ключи сервисов **не персистируются**: они хранятся только в памяти процесса приложения на время его работы и не записываются ни в SQLite, ни в защищённое хранилище Windows, ни в какие-либо файлы. При перезапуске приложения все аккаунты переходят в `Disconnected` и требуют повторной авторизации пользователем.
...
flowchart LR
    ...
    SEL -- "выбранные" --> ADD["Добавление:<br/>Person, ChannelContact,<br/>Conversation = Added"]
...
sequenceDiagram
    ...
    AD-->>APP: токен, AccountId (может отсутствовать)
    APP->>APP: хранить токен в памяти процесса
    APP->>DB: INSERT ConnectedAccount
    ...
    APP->>DB: BEGIN, Person / ChannelContact / Conversation(Added), RelationChangeLog, COMMIT
...
flowchart TB
    ...
    SPLIT -->|Added| SAVE["UPSERT messages<br/>по (conversation_id, external_id)"]
...
- Исходящее сообщение, пришедшее при синхронизации, сопоставляется с локальным по `external_id` и не дублируется.

### 9.4. Загрузка единого чата (US-06)

Единый чат собирается из `k` личных диалогов собеседника (обычно 1–5). Загрузка страницы:

1. Найти диалоги: `conversations` со `status = Added`, `type = Personal`, `channel_contact_id` из идентичностей собеседника.
2. Для каждого диалога выполнить запрос по индексу `(conversation_id, sent_at DESC, id DESC)`:
   ```sql
   SELECT id, conversation_id, channel_type, direction, status, text, sent_at
   FROM messages
   WHERE conversation_id = @conversationId
     AND (sent_at, id) < (@cursorSentAt, @cursorId)
   ORDER BY sent_at DESC, id DESC
   LIMIT 50;
   ```
3. Слить `k` отсортированных списков (k-way merge) и взять первые 50.
4. Вернуть страницу и курсор = `(sent_at, id)` последнего элемента.

Почему так, а не одним запросом с `IN (...)`: SQLite при `conversation_id IN (...) ORDER BY sent_at` вынужден отсортировать все подходящие строки. Раздельные запросы используют индекс и читают не более `50 × k` строк независимо от длины истории. Это также избавляет от хранения `person_id` в `messages`, которое пришлось бы массово обновлять при merge/split.
...
Список «Чаты» строится агрегацией по небольшой таблице `conversations`: для личных диалогов — группировка по собеседнику (`MAX(last_message_at)`, `SUM(unread_count)`), для групп и broadcast — по диалогу; объединение сортируется по дате последнего сообщения с постраничной выдачей.
...
`client_message_id` генерируется клиентом и защищает от двойной отправки при повторе запроса.

Выбор канала по умолчанию:

1. канал последнего входящего сообщения от собеседника;
2. иначе `person.preferred_channel_contact_id`;
...
2. Перенести все `ChannelContact` из B в A.
...
2. В транзакции: создать B, перенести выбранные `ChannelContact`, записать `Split`.
...
sequenceDiagram
    ...
    API->>APP: пароль пустой? (хэш = хэш пустой строки)
    APP-->>API: passwordEnabled
    API-->>UI: passwordEnabled, authenticated = false
    U->>UI: пароль (или пустой ввод, если passwordEnabled = false)
    UI->>API: POST /api/auth/login
    API->>APP: вычислить KDF, сравнить хэш
    alt верно
        APP-->>API: сессия
...
`password_hash` в `user_profiles` всегда заполнен: пароль как таковой отключить нельзя, но можно сделать его пустым. `passwordEnabled` в API — не хранимое поле, а производное значение: `false`, если сохранённый хэш совпадает с хэшем пустой строки (с той же солью и KDF), иначе `true`. При `passwordEnabled = false` вход проходит без реального ввода пароля.
...
| ФТ-102 | Пароль включается, меняется и отключается пользователем; смена и отключение требуют текущий пароль. «Отключение» — это установка пустого пароля, отдельного флага в БД нет (см. 9.7). | Обязательно |
| ФТ-103 | Пока пароль не пуст, до входа UI показывает только экран входа, API возвращает 401 для всех методов, кроме `/api/auth/*`. | Обязательно |
...
| ФТ-204 | Отображение состояния аккаунта, дат подключения и последней синхронизации.                                        | Обязательно |
...
| ФТ-211 | Секрет аккаунта хранится только в памяти процесса и не персистируется. После перезапуска приложения все аккаунты переходят в `Disconnected` до повторной авторизации. | Обязательно |

### 10.3. US-03. Настройки

Объём сокращён в v2.1: ФТ-302, ФТ-305, ФТ-306 вынесены за пределы v1 (раздел 20), ФТ-308 упрощён до вкл/выкл без уровня детализации.

| ID     | Требование                                                                              | Приоритет   |
| ------ | --------------------------------------------------------------------------------------- | ----------- |
| ФТ-301 | Язык интерфейса и тема оформления.                                                      | Обязательно |
| ФТ-302 | Запуск вместе с Windows.                                                                | Вне v1      |
| ФТ-303 | Интервал фоновой синхронизации.                                                         | Обязательно |
| ФТ-304 | Настройки безопасности (ФТ-102, ФТ-106).                                                | Обязательно |
| ФТ-305 | Расположение файла БД с переносом существующего файла.                                  | Вне v1      |
| ФТ-306 | Правила хранения истории (например, не хранить старше N дней).                          | Вне v1      |
| ФТ-307 | Настройки аккаунта: синхронизация вкл/выкл, типы кандидатов, показываемые по умолчанию. | Обязательно |
| ФТ-308 | Техническое логирование: включение и выключение (без настройки уровня в v1).            | Обязательно |
...
| ФТ-604 | Лента единого чата из всех личных диалогов собеседника по `sent_at`.                       | Обязательно |
...
| ФТ-809 | Повторное открытие экрана выбора из раздела «Каналы»; при повторном открытии в нём же видны диалоги в статусе `Pending`, появившиеся после подключения. | Обязательно |
| ФТ-810 | Действия «Добавить» и «Игнорировать» для диалогов, появившихся после подключения (`Pending`), — в экране выбора кандидатов, без отдельного раздела UI (v2.1). | Обязательно |
| ФТ-811 | Игнорированные диалоги не предлагаются повторно; их можно найти и вернуть через фильтр «Игнорированные» в том же экране (v2.1: без отдельного списка). | Обязательно |
...
Поиск (бывшие ФТ-951…953, US-09) и уведомления (бывшие ФТ-961…962, US-10) исключены из v1 в v2.1 и перенесены в раздел 20 «Перспективы развития».
...
Отдельного пункта «Новые диалоги» в v2.1 нет: диалоги в статусе `Pending` обрабатываются в экране «Выбор кандидатов» (см. US-08).

| Экран                | Содержание                                                                                                                 | US           |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------ |
| Вход                 | Ввод пароля                                                                                                                | US-01        |
...
| Выбор кандидатов     | Вкладки по типам (включая `Pending`), поиск, множественный выбор, подсказки совпадений, глубина истории, «Игнорированные» | US-08        |
| Предложения          | Пары, причины, «Объединить» / «Отклонить»                                                                                 | US-05        |
| Настройки            | Общие, безопасность, аккаунты, логирование (вкл/выкл)                                                                     | US-03        |
...
| ID    | Требование                                                                                                                                                                                       | Приоритет   |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| БЗ-01 | Пароли, токены, API keys, refresh tokens каналов никогда не записываются на диск в открытом виде.                                                                                                | Обязательно |
| БЗ-02 | Секреты каналов хранятся только в памяти процесса на время его работы и не персистируются на диск (ни в Windows Credential Manager/DPAPI, ни иным способом); при перезапуске приложения требуется повторная авторизация каждого аккаунта.  | Обязательно |
| БЗ-03 | Секреты не сохраняются в БД и не выводятся в логи или доменные исключения в явном виде.                                                                                                          | Обязательно |
...
| Поиск                       | Поиск по собеседникам и по тексту сообщений (US-09, раздел 20)                          |
| Уведомления                 | Уведомления о новых сообщениях (US-10, раздел 20)                                       |
| Настройки                   | Смена расположения файла БД, автозапуск с Windows, правила хранения истории, настраиваемый уровень логирования (раздел 20) |
| UI                          | Отдельный раздел «Новые диалоги» — обрабатывается в экране «Выбор кандидатов» (US-08)   |
...
| 8. Группы и broadcast      | Групповые чаты, участники, отправка в группу, обработка `Pending`-диалогов в экране выбора кандидатов | Полная поддержка типов диалогов                       | US-06, US-07, US-08 |
...
| ПР-15 | Новые диалоги не добавляются автоматически; они доступны для добавления или игнорирования через экран «Выбор кандидатов». | US-08 |
...
US-09 (поиск) и US-10 (уведомления) исключены из v1 в v2.1 — см. раздел 20.
...
| ОВ-05 | Поиск по тексту сообщений: FTS5 в v1 или позже?              | А) FTS5 в v1. Б) Отложить.                                                                                 | **Решено (v2.1):** поиск исключён из v1 целиком (см. ОВ-09, раздел 20). Вопрос о FTS5 актуален при реализации US-09 в следующей версии.                                                                     |
...
| ОВ-09 | Подтвердить US-09 (поиск) и US-10 (уведомления).             | А) Включить в v1. Б) Исключить из v1, оставить как перспективу.                                            | **Решено (v2.1):** Б. Обе исключены из v1 и перенесены в раздел 20 без привязки к версии.                                                                                                                    |
...
- **US-09. Поиск** — по собеседникам и по тексту сообщений (полнотекстовый, например FTS5, см. ОВ-05), исключён из v1 в v2.1;
- теги и заметки;
- **US-10. Уведомления** — по каналу, диалогу, собеседнику, исключены из v1 в v2.1;
- настраиваемое расположение файла БД, автозапуск с Windows, правила автоматического хранения/удаления истории, настраиваемый уровень логирования — вынесены из v1 в v2.1;
